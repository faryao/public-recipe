(function(global, factory) {
	...
	var PROTECTED_WORDS_PATTERN = new RegExp(["公視+", "影劇館+"].map((word) => word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|"), "g");
	var PROTECTED_WORD_PLUS_MASK_BEFORE_WORD = new RegExp(`(\uE022)(?=[${CJK}${AN}${LEFT_BRACKETS_BASIC}])`, "g");
	var PROTECTED_WORD_PLUS_MASK = /\uE022/g;
	...
	var junctionVerdictCache = /* @__PURE__ */ new Map();
	var JUNCTION_VERDICT_CACHE_MAX = 4096;
	function needsBoundarySpace(currentTail, nextFirst) {
		...
		const verdict = pangu$1.spacingText(junction).endsWith(` ${nextFirst}`) && !isQuoteNextToCjk(currentTail.slice(-1), nextFirst);
		if (junctionVerdictCache.size >= JUNCTION_VERDICT_CACHE_MAX) junctionVerdictCache.clear();
